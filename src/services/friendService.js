...
export default {
    get_list: async param => {
        ...
        //     finnalyResult = {
        //         rows: uniqueFriendList, // Dữ liệu danh sách bạn bè
        //         page: page, // Trang hiện tại
        //         perPage, // Số lượng phần tử trên mỗi trang
        //         // count, // Tổng số bạn bè trong cơ sở dữ liệu
        //     };
        //     return finnalyResult;
        //     // finnalyResult = {
        //     //     rows: uniqueFriendList,
        //     //     pagination: {
        //     //         page,
        //     //         perPage,
        //     //     }
        ...
        try {
            ...
            const perPage = range[1] - range[0] + 1; // Số lượng phần tử trên mỗi trang
            const offset = range[0]; // Vị trí bắt đầu

            const att = filterHelpers.atrributesHelper(attributes); // Xử lý attributes nếu cần

            const [result] = await Promise.all([
                MODELS.findAndCountAll(friend, {
                    where: {
                        [Op.or]: [
                            { user1_id: userId },
                            // { user2_id: userId }
                    ...
                    order: sort,
                    offset, // Bắt đầu từ vị trí range[0]
                    limit: perPage, // Số lượng phần tử trên mỗi trang
                    attributes: att,
                    logging: true, // Ghi log truy vấn (tuỳ chọn)
                }),
            ]);
            ...
            const friendList = friends
                ...
                .filter(friend => friend !== null); // Loại bỏ các giá trị null
            // console.log("friendList:", JSON.stringify(friendList, null, 2));
            // Loại bỏ các cặp trùng lặp (dựa trên id)
            // const uniqueFriendList = Array.from(
            //     new Map(friendList.map(friend => [friend.id, friend])).values()
            // );
            // const total = uniqueFriendList.length;
    ...
    handleRequestFriend: async param => {
        ...
        if (status === 'accept') {
            ...
            for (let pair of friendPair) {
                const existingFriendship = await MODELS.findOne(friend, {
                    where: {
