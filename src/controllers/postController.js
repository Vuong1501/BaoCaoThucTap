...
export default {
            ...
            postService
                ...
                .then(posts => {
                    if (posts && posts.rows.length > 0) {
                        const dataOutput = {
                            result: posts.rows, // Trả về danh sách bài viết
                            pagination: {
                                page: posts.page, // Trang hiện tại
                                perPage: posts.perPage, // Số bản ghi mỗi trang
                                total: posts.count // Tổng số bản ghi
                            },
                            success: true,
                            errors: [],
                            messages: []
    ...
    getPublicAndFriendPost: (req, res, next) => {
        try {
            ...
            console.log("userId", userId);
            console.log("differentUserId", differentUserId);
                    ...
                    const dataOutput = {
                        result: {
                            ...
                            pagination: {
                                ...
                                total: data.total[0][0].total // Tổng số bản ghi
                            }
                        },
                        success: true,
                        errors: [],
